...
document.addEventListener('DOMContentLoaded', () => {
  ...
  faqItems.forEach(item => {
    ...
    item.querySelector('.faq-q').addEventListener('click', () => {
      ...
      faqItems.forEach(i => { i.classList.remove('is-open'); setFaqHeight(i); });
      if (!wasOpen) { item.classList.add('is-open'); setFaqHeight(item); }
    });
  });
  window.addEventListener('resize', () => {
    faqItems.forEach(item => { if (item.classList.contains('is-open')) setFaqHeight(item); });
  });
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      // TODO: replace with a real submission (fetch to your form endpoint)
      form.reset();
